...
SPECIAL_OBJECT_KEYS = ["label", "kind", "background", "style", "freeform"]
...
class Builder:
    ...
    def add_search_content(self, slug, content, title=None, description=None, kind=None):
        ...
        self.search_dict[slug]["content"] = (
            set() if "content" not in self.search_dict[slug] else self.search_dict[slug]["content"]
        )
    ...
    def save_search_list(self, path=None):
        path = (
            os.path.join(self.output_dir if self.output_dir is not None else "", "_data", "search.yml")
            if path is None
            else path
        )
        print("Writing search dict to", path)
        for key in self.search_dict:
            self.search_dict[key]["content"] = (
                " ".join(self.search_dict[key]["content"]) if "content" in self.search_dict[key] else ""
            )
    ...
    def build_person(self, person, assets_src_dir=None, assets_target_dir=None, search_slug=None):
        ...
        if "image" in person:
            ...
            target = (
                person["image"]
                if assets_target_dir is None
                else os.path.join(f"{assets_target_dir}", f'{person["image"]}')
            )
    ...
    def build_link(self, link, assets_src_dir=None, assets_target_dir=None, search_slug=None, add_search_item=True):
        ...
        if "notebook" in link or "md" in link:
            ...
            link["link"] = prepend_baseurl(
                index_link if self.init_index[1] != index_link else self.init_index[0], baseurl=self.base_url
            )
        ...
        if (
            add_search_item
            and ("kind" not in link or link["kind"] != "person")
            and ("link" in link and not link["link"].startswith("#"))
        ):
            self.add_search_item(
                slug=link["link"],
                url=link["link"],
                title=link.get("text", link.get("kind", "External link")),
                description=link.get("description", None),
                kind=link.get("kind", "External"),
        ...
        if template_apply == "value":
            for key, value in obj.items():
                if key in SPECIAL_OBJECT_KEYS:
                    continue
                obj[key] = self.process_template(
                    value,
                    template=template,
                    template_apply="whole",
            ...
            result_content = self.templates_environment.get_template(template).render(
                markdown=functools.partial(
                    build_markdown, builder=self, extensions=self.markdown_extensions, process_html=False
                ),
                obj=obj,
                patch=patch,
            )
            result["content"] = process_html(
                builder=self,
                raw_html=result_content,
                assets_src_dir=assets_src_dir,
                assets_target_dir=assets_target_dir,
                search_links=search_links,
            )
    ...
    @patch_decorator
    def build_object(
        self, obj, image_key=None, assets_src_dir=None, assets_target_dir=None, search_slug=None, search_links=False
    ):
            ...
            if template_dict:
                ...
                for idx, item in enumerate(obj["content"]):
                    obj["content"][idx] = self.build_index(
                        ...
                        assets_target_dir=assets_target_dir,
                        search_slug=search_slug,
                        search_content=search_slug is not None,
                        search_links=search_links,
                    )
            else:
    ...
    def build_index(
        ...
        assets_src_dir=None,
        assets_target_dir=None,
        target_data_file: str = None,
        index_type="index",
        search_slug: str = None,
        search_links: th.Optional[bool] = False,
        search_content: th.Optional[bool] = False,
        init_index=False,
    ):
        ...
        if index is None:
            ...
            search_slug = (
                prepend_baseurl(
                    remove_ending(
                        index_file if target_data_file is None else target_data_file, [".yml", ".yaml", ".html", ".md"]
                    ),
                    baseurl=self.base_url,
                )
            ...
            assert os.path.isfile(index_file), f"{index_type.capitalize()} file {index_file} could not be found!"
            ...
            if init_index:
                self.init_index = (
                    remove_ending(index_file if target_data_file is None else target_data_file, [".yml", ".yaml"]),
                    remove_ending(index_file, [".yml", ".yaml"]),
                )
        ...
        assert isinstance(index, dict), f"index is supposed to be an object, got {type(index)}"
        # page info
        ...
        if search_links is None and search_content is None:
            ...
            if index["search"]["content"] or index["search"]["links"]:
                title = (
                    index["header"]["title"]
                    if "header" in index and "title" in index["header"]
                    else index.get("title", index_type.capitalize())
                )
        ...
        if "nav" in index:
            ...
            if isinstance(index["nav"], dict) and "brand" in index["nav"]:
                index["nav"]["brand"] = self.build_link(
                    ...
                    assets_src_dir=assets_src_dir,
                    assets_target_dir=assets_target_dir,
                    search_slug=search_slug if search_content else None,
                    add_search_item=search_links,
                )
            else:
